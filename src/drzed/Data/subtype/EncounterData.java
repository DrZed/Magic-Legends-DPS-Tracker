...
public class EncounterData {
    ...
    public static void loadEncounter(String f) throws IOException {
        ...
        while ((lin = r.readLine()) != null) {
            ...
            if (tlin.startsWith("time")) {
                encounter.time = Long.parseLong(tlin.split("=")[1]);
            } else if (tlin.startsWith("duration")) {
                encounter.duration = Long.parseLong(tlin.split("=")[1]);
            ...
            } else if (tlin.startsWith("allEntities=[")) {
                while (!(tlin = r.readLine().trim()).equals("]")) {
                    Entity e = parseEntity(r, tlin.split("=")[0]);
                    if (e == null) continue;
                    encounter.allEntities.put(e.ID, e);
                }
            }/* else if (tlin.equals("}")) {
                Main.export("TESTING");  //Succeed in exact copy of encounter exported
            }*/
            ...
            if (nlin.endsWith("=[")) {
                ...
                while (!(nlin = r.readLine().trim()).equals("]")) {
                    ...
                    if (nlin.startsWith("baseDamage")) {
                        ability.baseDamage = Double.parseDouble(nlin.split("=")[1]);
                        continue;
                    }
                    if (nlin.startsWith("Damage")) {
                        ability.Damage = Double.parseDouble(nlin.split("=")[1]);
                        continue;
                    }
